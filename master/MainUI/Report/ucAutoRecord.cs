        ...
        private void FillManualExcelTemplate(string filePath, ManualReportHeaderInfo headerInfo)
        {
            ...
            for (int pageIndex = 0; pageIndex < totalPages; pageIndex++)
            {
                ...
                for (int i = 0; i < pageRecords.Count; i++)
                {
                    ...
                    SetCell(rowA, 17, rec.FrontTurbochargerTempIn); 
                    SetCell(rowB, 17, rec.AfterTurbochargerTempIn);   // 59/60 涡轮前 前/后
                    SetCell(rowA, 18, rec.FrontTurbochargerTempOut); 
                    SetCell(rowB, 18, rec.AfterTurbochargerTempOut);  // 61/62 涡轮后 前/后
                }
            }
