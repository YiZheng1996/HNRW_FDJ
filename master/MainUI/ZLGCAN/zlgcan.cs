...
namespace ZLGCAN
{
    ...
    public class Method
    {
        ...
        #region 自定义常用方法============================================
        //设置波特率
        public static bool SetBaudrate(IntPtr device_handle_,int channel_index_, UInt32 baud)
        {
            ...
            return 1 == ZCAN_SetValue(device_handle_, path, Encoding.ASCII.GetBytes(value));
        }

       public static bool Reset(IntPtr device_handle_,IntPtr channel_handle_, int channel_index_)
        {
        ...
        public static uint MakeCanId(uint id, int eff, int rtr, int err)//1:extend frame 0:standard frame
        {
            ...
            UInt32 AA = ueff << 31;
            uint rst = id | ueff << 31 | urtr << 30 | uerr << 29;
            return rst;
        }
        ...
        public static bool IsERR(uint id)//1:error frame 0:normal frame
        {
